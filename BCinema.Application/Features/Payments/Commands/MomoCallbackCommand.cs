...
public class MomoCallbackCommand : IRequest<string>
{
        ...
        IFoodRepository foodRepository) : IRequestHandler<MomoCallbackCommand, string>
    {
        ...
        private async Task<bool> SendMailTicket(string email, Payment payment, SeatSchedule seatSchedule,
            CancellationToken cancellationToken)
        {
            ...
            foreach (var item in payment.PaymentDetails)
            {
                ...
                var seatS = await seatScheduleRepository.GetSeatScheduleByIdAsync((Guid)item.SeatScheduleId, cancellationToken)
                            ?? throw new NotFoundException(nameof(SeatSchedule));
            ...
            var emailBody = htmlTemplate
                ...
                .Replace("{TotalPrice}", totalPrices.ToString());
