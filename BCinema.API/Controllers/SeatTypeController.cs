    ...
    public class SeatTypeController : ControllerBase
    {
        ...
        public async Task<IActionResult> GetSeatTypes([FromQuery] SeatTypeQuery query)
        {
            try
            {
                ...
                return Ok(new PageResponse<IEnumerable<SeatTypeDto>>(true, "Get all seat types successfully",
                    seatTypes.Data, seatTypes.Page, seatTypes.Size, seatTypes.TotalPages, seatTypes.TotalElements));
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "An unexpected error occurred while getting all seat types.");
        ...
        public async Task<IActionResult> GetSeatTypeById(Guid id)
        {
            try
            {
                var seatType = await _mediator.Send(new GetSeatTypeByIdQuery { Id = id });
                return Ok(new ApiResponse<SeatTypeDto>(true, "Get seat type by id successfully", seatType));
            }
            ...
            {
                _logger.LogError(ex, "An unexpected error occurred while getting seat type by id.");
                return StatusCode(500, new ApiResponse<string>(false, "An unexpected error occurred."));
            }
        }
        ...
        public async Task<IActionResult> DeleteSeatType(Guid id)
        {
            try
            {
                var seatType = await _mediator.Send(new DeleteSeatTypeCommand { Id = id });
                return Ok(new ApiResponse<SeatTypeDto>(true, "Seat type deleted successfully", seatType));
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "An unexpected error occurred while deleting seat type.");
                return StatusCode(500, new ApiResponse<string>(false, "An unexpected error occurred."));
            }
        }
    }
